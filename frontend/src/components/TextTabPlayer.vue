...
export default defineComponent({
    ...
    methods: {
        ...
        async ensureYoutube() {
            if (this.youtube) { this.youtube.cueVideoById(this.source.slice(8)); return; }
            if (!window.YT) {
                await new Promise((resolve) => { window.onYouTubeIframeAPIReady = resolve; const script = document.createElement("script"); script.src = "https://www.youtube.com/iframe_api"; document.head.appendChild(script); });
            }
            this.youtube = new window.YT.Player(this.$refs.youtube, { height: "180", width: "320", videoId: this.source.slice(8), events: { onStateChange: (event) => { if (event.data === window.YT.PlayerState.ENDED) { this.playing = false; clearInterval(this.scrollTimer); } } } });
        },
        onManualScroll() { this.manualUntil = Date.now() + 3000; },
        startScroll() {
            clearInterval(this.scrollTimer);
            this.scrollTimer = setInterval(() => {
                if (!this.playing || Date.now() < this.manualUntil) return;
                let time = this.source === "none" ? (performance.now() - this.startedAt) / 1000 : 0; let total = this.duration;
                if (this.source.startsWith("audio-")) { time = this.$refs.audio.currentTime; total = this.$refs.audio.duration || total; }
                if (this.source.startsWith("youtube-") && this.youtube) { time = this.youtube.getCurrentTime(); total = this.youtube.getDuration() || total; }
...
<template>
    <section class="text-tab" :class="{ fullscreen }">
        ...
        <footer>
            <select class="form-select" v-model="source" @change="selectSource"><option value="none">Timer only</option><option v-for="item in sources" :key="item.value" :value="item.value">{{ item.label }}</option></select>
            ...
            <audio ref="audio" @ended="playing = false" @pause="playing = false" hidden></audio><div ref="youtube"></div>
        </footer>
    </section>
</template>

<style scoped lang="scss">
.text-tab { min-height: 100dvh; background: #101217; color: #eee; display: flex; flex-direction: column; }
header, footer { padding: 12px 16px; display: flex; gap: 12px; align-items: center; justify-content: space-between; background: #1b2029; }
h1 { font-size: 1.25rem; margin: 0; } h2 { font-size: 1rem; color: #aaa; margin: 0; }
pre { flex: 1; overflow: auto; margin: 0; padding: 24px 16px 96px; font: 16px/1.6 "Courier New", monospace; white-space: pre; -webkit-overflow-scrolling: touch; }
footer { position: sticky; bottom: 0; justify-content: flex-start; } footer .form-select { max-width: 280px; } footer .form-control { width: 90px; }
.fullscreen { position: fixed; inset: 0; z-index: 2000; }
@media (min-width: 768px) { pre { font-size: 18px; padding: 32px 48px 104px; } }
</style>
